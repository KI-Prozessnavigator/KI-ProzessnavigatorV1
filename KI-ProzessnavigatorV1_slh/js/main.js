...
function initScrollAnimations() {
    if (!('IntersectionObserver' in window)) {
        document.querySelectorAll('[data-animate], .section__header').forEach(el => {
...
function initUseCasesSlider() {
    ...
    if (window.innerWidth <= 1024) {
        ...
        slider.querySelectorAll('.usecase-card--clone').forEach((clone) => clone.remove());
        return;
    }
