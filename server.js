...
app.get(/\.html$|^\/$|^\/[^.]+$/, (req, res, next) => {
    let filePath = path.join(__dirname, req.path === '/' ? 'index.html' : req.path.replace(/\/$/, '') + '.html');
    ...
    html = html.replace(/<!--#include virtual="([^"]+)"\s*-->/g, (_, inc) => {
      const incPath = path.join(__dirname, inc);
      try { return fs.readFileSync(incPath, 'utf8'); } catch { return ''; }
    });
